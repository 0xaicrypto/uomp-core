...
`uomp-mvp` 已提供可运行的 Gateway 参考实现：

```bash
# 1. 确保本地 Auth + Guard 服务已启动
pnpm --filter @uomp/server start

# 2. 生成 CA / Gateway 服务端证书 / 客户端证书
./scripts/generate-gateway-certs.sh

# 3. 把客户端指纹加入 ~/.uomp/remote-profile.json 的 agent_allowlist

# 4. 启动 Gateway
node apps/gateway/dist/index.js

# 5. 创建 remote session 并走通 end-to-end
./scripts/test-gateway-remote.sh
```

当前 Gateway 已支持：mTLS 终结、远程 Token 校验、Memory Guard 转发、`/v1/audit` 查询、临时 Payload 缓存（未加密，Phase 2 实现 E2E）。
